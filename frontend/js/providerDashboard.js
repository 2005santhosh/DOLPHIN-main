/* ==========================================
   PROVIDER DASHBOARD - FULL FRONTEND LOGIC
   ========================================== */
...
// Force ID to String immediately
const userId = (user._id || user.id)?.toString();

if (!token) console.warn("No token found. User might not be logged in.");
...
// Generic Helper to handle Auth headers and errors (Fallback if api.js isn't loaded)
async function apiCall(endpoint, method = 'GET', body = null, base = API_URL) {
...
// Helper to identify Request Type (Sent vs Incoming)
function identifyRequestType(req, currentUserId) {
    ...
    // 2. Fallback: Check Sender ID
    const sender = (req.senderId?._id || req.senderId)?.toString();
    if (sender) return sender === currentUserId ? 'sent' : 'incoming';

    // 3. Heuristic: Providers usually send 'servicesOffered'
    if (req.servicesOffered && req.servicesOffered.length > 0) return 'sent';
...
async function loadDashboard() {
    try {
        ...
        // 2. Load Stats
        const requests = await window.api.getMyRequests();
        ...
        try {
            const eligible = await window.api.getEligibleFounders();
...
async function loadFounders() {
    ...
    try {
        // 1. Fetch Founders
        const founders = await window.api.getEligibleFounders();
        
        // 2. Fetch My Requests to check status
        const requests = await window.api.getMyRequests();
        
        // 3. Create Maps: startupId -> request object
    ...
    if (mySentRequest) {
        ...
        if (status === 'accepted') {
            ...
            actionsHtml = `
              ...
              <button class="btn btn-primary" onclick="navigateToPage('requests')">View Requests</button>
            `;
        } else {
...
// ==========================================
// 8. REQUESTS LOGIC (SPLIT INCOMING / SENT)
// ==========================================
...
window.updateRequest = async function(requestId, newStatus) {
    ...
    try {
        await window.api.updateIntroRequest(requestId, newStatus);
...
document.getElementById('send-request').addEventListener('click', async () => {
    ...
    try {
        const result = await window.api.sendProviderRequest(currentStartupId, message, services);
...
// ==========================================
// 10. FOUNDER PROFILE MODAL
// ==========================================

window.viewFounderProfile = function(startupId) {
    const profileModal = document.getElementById('profile-modal');
    // We need to find the founder from the current list loaded in memory
    // Assuming loadFounders stored data in window.currentFoundersList
    // If not, we just grab from the cached list
    
    // Ideally, we should fetch details from API, but for now we use cached list
    const founders = window.currentFoundersList || [];
    const target = founders.find(f => f._id === startupId);
...
document.getElementById('upload-picture-btn')?.addEventListener('click', async () => {
    ...
    btn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" style="animation: spin 1s linear infinite;"><path d="M21 12a9 9 0 1 1-6.219-8.56"></path></svg> Uploading...`;
    btn.disabled = true;
...
document.getElementById('update-profile-btn')?.addEventListener('click', async () => {
    ...
    try {
        ...
        if (res.status === 404) {
             res = await window.api.updateProfile({ name }); 
        }
...
// Connect clear functions to nav clicks
document.querySelector('[data-page="requests"]')?.addEventListener('click', () => clearSidebarBadge('sidebar-req-badge'));
document.querySelector('[data-page="chat"]')?.addEventListener('click', () => clearSidebarBadge('sidebar-chat-badge'));
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        if (requestModal.classList.contains('active')) requestModal.classList.remove('active');
        if (profileModal.classList.contains('active')) profileModal.classList.remove('active');
        if (document.getElementById('legal-modal').classList.contains('active')) closeLegalModal();
