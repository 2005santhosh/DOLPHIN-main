   // ==========================================
    ...
    window.toggleLegalSection = function(element) {
        ...
        // Close all other open sections (optional accordion behavior)
    ...
    async function loadDashboard() {
      try {
        ...
        const userRes = await fetch('/api/auth/profile', { headers: { 'Authorization': `Bearer ${token}` } });
    ...
    document.getElementById('profile-form').addEventListener('submit', async (e) => {
      ...
      try {
        ...
        if (existingStartup) { await api.updateStartup(startupData); alert('Profile updated successfully!'); } 
        else { await api.createStartup(startupData); alert('Startup created successfully!'); }
    ...
    async function openStageValidationModal(stageKey) {
      ...
      try {
        const response = await fetch(`/api/founder/validate-stage/${stageKey}/questions`, { headers: { 'Authorization': `Bearer ${token}` } });
        const data = await response.json();
        if (!data.success) { alert(data.message || 'Failed to load questions'); return; }
      ...
      } catch (error) { alert('Failed to load validation questions'); }
    }
    ...
    async function submitStageValidation(e) {
      ...
      for (let elem of answerElements) { if (elem.value.trim() === '') { alert('Please answer all questions'); return; } }
      ...
      try {
        ...
        if (result.success) {
          ...
          alert(result.isValidated ? `✓ Stage Validated (${result.validationScore}%)` : `Stage Score: ${result.validationScore}% (need 70%)`);
          loadStages(); loadDashboard();
        } else { alert(`Error: ${result.message || 'Validation failed'}`); }
      } catch (error) { formEl.style.display = 'block'; loadingEl.style.display = 'none'; alert('Failed to submit validation'); }
    }
    ...
    async function showStageValidationResults(stageKey) {
      try {
        ...
        if (!rec || !rec.completedAt) { alert('No results yet.'); return; }
      ...
      } catch (error) { alert('Failed to load results'); }
    }

       // ==========================================
    // TASKS & RESOURCES (CORRECTED)
    // ==========================================
    async function loadRoadmapTasks() {
        ...
        try {
            ...
            container.innerHTML = res.tasks.map(task => {
                ...
                // FIX: Added closing </div> at the very end
                return `
    ...
    async function completeTask(taskKey) {
        ...
        } catch (err) { alert(err.message || 'Failed to complete task'); }
    }
    ...
    async function loadAnalytics() {
      ...
      } catch(e) { grid.innerHTML = '<p style="text-align: center; color: var(--danger);">Failed to load analytics.</p>'; }
    }
    ...
    function initCharts(perf, tasks) {
      ...
      if(ctx2) new Chart(ctx2, { type: 'doughnut', data: { labels: ['Completed', 'Pending'], datasets: [{ data: [tasks.completed, tasks.pending], backgroundColor: ['#10b981', '#e5e7eb'] }] }, options: { responsive: true, maintainAspectRatio: false } });
       ...
       // ==========================================
    // INVESTORS & PROVIDERS (UPDATED) loadfounder
    // ==========================================
    async function loadInvestorsProviders() {
      ...
      try {
        ...
        // Updated to show specific tags in preview cards
        contentEl.innerHTML = `
    ...
    async function loadProviders() {
      ...
      try {
        ...
        // FIX: Update Local Storage so the name persists on refresh
        const updatedUser = { ...user, name: data.user.name }; // Merge old user data with new name
    ...
    document.getElementById('submit-founder-request')?.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch('/api/founder/send-request', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        ...
        } catch(e) { alert('❌ ' + e.message); btn.disabled = false; btn.textContent = 'Send Request'; }
    });
    ...
    // ==========================================
       ...
       // Helper to navigate to chat page and open specific chat
    window.navigateToChat = function(partnerId, partnerName, partnerPic) {
        // 1. Close the requests page
        document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
        ...
        // 4. Trigger the Chat Window opening
        // Use .toString() to ensure ID is a string
      ...
      try {
        ...
        if (type === 'investor') {
            detailsHtml += `
              ...
              <div><strong style="font-size:0.8rem; color:#888; display:block; margin-bottom:4px;">Rating</strong>⭐ ${data.rating || '0.0'}</div>
        ...
        // FIX: Chat button calls navigateToChat
    ...
    async function submitRating(targetUserId) {
      ...
      try {
        const res = await fetch('/api/founder/rate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        ...
        // Optional: Update the rating display in the modal without closing
        document.querySelector('#detail-modal-body [style*="Rating"]').innerHTML = `⭐ ${data.averageRating || selectedRating}.0`;
      } catch (err) { alert('❌ ' + err.message); }
    }
    ...
    window.closeDetailModal = closeDetailModal;
        ...
        list.innerHTML = '<p style="text-align:center;">Loading...</p>';
        try {
            const res = await fetch('/api/founder/requests', { headers: { 'Authorization': `Bearer ${token}` } });
            ...
            reqs.forEach(r => {
                ...
                // FIX: Ensure providerId is a string
       ...
       async function loadSentRequests() {
        ...
        list.innerHTML = '<p style="text-align:center;">Loading...</p>';
        try {
            const res = await fetch('/api/founder/requests/sent', { headers: { 'Authorization': `Bearer ${token}` }});
            ...
            reqs.forEach(r => {
                ...
                div.className = 'list-item';
                const provider = r.providerId || {};
                // FIX: Ensure provider._id is extracted correctly
                const providerId = (provider._id || provider)?._id || provider._id || provider;
                const providerIdStr = providerId?.toString();
                const providerName = provider.name || 'Provider';
                const providerPic = provider.profilePicture ? (provider.profilePicture.startsWith('http') ? provider.profilePicture : `${window.location.origin}${provider.profilePicture}`) : null;
    ...
    async function handleRequest(id, status) {
      try {
        const res = await fetch(`/api/founder/requests/${id}/${status === 'accepted' ? 'accept' : 'reject'}`, { method: 'PUT', headers: { 'Authorization': `Bearer ${token}` } });
    ...
    async function loadConversations() {
        try {
            ...
            list.innerHTML = '';
            if (!convs.length) { list.innerHTML = '<p style="padding:1rem; text-align:center; color:#666;">No conversations yet.</p>'; return; }
            convs.forEach(c => {
                const div = document.createElement('div');
                div.className = 'chat-item';
                const partnerName = c.name || 'Chat';
                let avatarHtml = c.profilePicture ? `<img src="${c.profilePicture.startsWith('http') ? c.profilePicture : window.location.origin + c.profilePicture}" class="chat-avatar" alt="${partnerName}">` : `<div class="chat-avatar">${partnerName.charAt(0)}</div>`;
    ...
    window.sendMessage = async function() {
        ...
        container.scrollTop = container.scrollHeight;
        input.value = '';
        try { await chatApiCall('/send', 'POST', { receiverId: currentChatPartnerId, content }); } catch(e) { alert('Failed to send message'); }
    };
            ...
            try {
                const res = await fetch('/api/auth/upload-profile-picture', {
                    ...
                    headers: {
                        ...
                        // Note: Do NOT set 'Content-Type' header manually for FormData; browser handles it with boundary
                    },
                    body: formData
                });
